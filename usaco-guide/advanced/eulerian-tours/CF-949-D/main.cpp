...
template<bool directed> struct Euler {
	...
	void ae(int a, int b) {
		...
		adj[a].eb(b,M); if (!directed && a!=b) adj[b].eb(a,M); }
...
void solve() {
	int n; read(n);
	auto check=[&](ll x) {
		return (x&1 ? x+x*(x-1)/2 : x+x*(x-2)/2+1)>=n-1;
	};
	int a=FIRSTTRUE(check, 1, sz(prime));
	Euler<false> euler; euler.init(a);
	if (a&1) {
		FOR(a) FOR(j, i, a) euler.ae(i, j);
	} else {
		FOR(i, 0, a, 2) FOR(j, i+2, a) euler.ae(i, j);
		FOR(i, 1, a, 2) FOR(j, i+1, a) euler.ae(i, j);
		FOR(a) euler.ae(i, i);
		euler.ae(0, 1);
	}
	vt<pii> seq=euler.solve();
	vt<int> ans;
	FOR(n) ans.pb(prime[seq[i].f]);
	print(ans);
}
