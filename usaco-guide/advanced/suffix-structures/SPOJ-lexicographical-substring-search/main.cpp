...
int main() {
	...
	int m, y; read(m);
	int n=sz(x);
	SuffixArray sa; sa.init(x);
	vt<int> cnt(n);
	FOR(n) {
		cnt[i]=n-sa.sa[i+1]-sa.lcp[i];
	}
	vt<int> pre(n+1); pre[0]=0;
	FOR(n) pre[i+1]=pre[i]+cnt[i];
	FOR(m) {
		read(y); --y;
		auto it=prev(upper_bound(all(pre), y));
		int idx=it-pre.begin();
		print(x.substr(sa.sa[idx+1], sa.lcp[idx]+y-*it+1));
	}
}