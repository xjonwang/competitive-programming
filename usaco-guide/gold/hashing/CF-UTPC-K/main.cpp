...
int main() {
	...
	int n; read(n);
	vt<str> v(n); read(v);
	int t=0; FOR(n) t+=sz(v[i]);
	vt<HashRange> hr(n); FOR(n) hr[i].add(v[i]);
	vt<vt<int>> c(n, vt<int>(n));
	vt<bool> e(n, 0);
	FOR(n) {
		FOR(j, n) {
			if (i==j) continue;
			int m=min(sz(v[j]), sz(v[i]));
			bool sub=0;
			if (sz(v[j])==m) {
				FOR(k, sz(v[i])-m+1) {
					if (hr[j].hash(0, m-1)==hr[i].hash(k, k+m-1)) {
						e[j]=1;
						sub=1;
						break;
					}
			...
			if (sub) continue;
			c[i][j]=0;
			FOR(k, m) if (hr[i].hash(sz(v[i])-1-k, sz(v[i])-1)==hr[j].hash(0, k)) c[i][j]=k+1;	
		}
	}
	vt<int> p(n); iota(all(p), 0);
	int ans=t;
	do {
		int a=0;
		FOR(n-1) a+=e[p[i]] ? sz(v[p[i]]) : c[p[i]][p[i+1]];
		umin(ans, t-a);
