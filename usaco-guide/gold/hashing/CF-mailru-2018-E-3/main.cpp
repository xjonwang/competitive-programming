...
int main() {
	...
	HashRange hr; hr.add(b);
	int ans=0;
	FOR(l0, 1, (m-1)/cnt0+1) {
		if ((m-l0*cnt0)%cnt1) continue;
		int l1=(m-l0*cnt0)/cnt1, k=0, st0=-1, st1=-1;
		bool v=1;
		FOR(j, n) {
			if (a[j]=='0') {
				if (st0==-1) st0=k;
				if (hr.hash(st0, st0+l0-1)!=hr.hash(k, k+l0-1)) {
					v=0;
					break;
				}
				k+=l0;
				...
				if (st1==-1) st1=k;
				if (hr.hash(st1, st1+l1-1)!=hr.hash(k, k+l1-1)) {
					v=0;
					break;
				}
				k+=l1;
		...
		if (hr.hash(st0, st0+l0-1)==hr.hash(st1, st1+l1-1)) v=0;
		ans+=v;
	}
	print(ans);
}