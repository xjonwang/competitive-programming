...
struct DSU {
	...
	bool unite(int x, int y) {
		x=get(x), y=get(y);
		if (x==y) return true;
		if (o[x]!=-1 && o[y]!=-1 && get(o[x])==y && get(o[y])==x) return false;
		if (e[x]>e[y]) swap(x, y);
		e[x]+=e[y];
		e[y]=x;
	...
	bool oppose(int x, int y) {
		x=get(x), y=get(y);
		if (x==y) return false;
		if (o[x]==-1) o[x]=y;
		else if (!unite(o[x], y)) return false;
		if (o[y]==-1) o[y]=x;
		else if (!unite(o[y], x)) return false;
		return true;
	}
};
...
void solve() {
	...
	FOR(n) {
		FOR(j, i+1, n) {
			...
			if (v[i][j]<v[j][i]) {
				if (!dsu.unite(i, j)) swap(v[i][j], v[j][i]);
			} else {
				if (dsu.oppose(i, j)) swap(v[i][j], v[j][i]);
			}
		}
	}
	EACH(r, v) print(r);
