...
struct ST {
	...
	void modify(int i, int x) {
		for (v[i+=n]=x; i/=2;) v[i]=max(v[i*2], v[i*2+1]);
	}
...
struct HLD {
	ST seg;
	vt<vt<int>> adj;
	vt<int> d, size, par, pos, chain; 
	int n, st, ti;
	HLD(int n, int st=0) : n(n), st(st), ti(0), adj(n), d(n), size(n), par(n), pos(n), chain(n), seg(n) {};
	void ae(int x, int y) {
	...
	int query(int u, int v) {
		...
		for (; chain[u]!=chain[v]; v=par[chain[v]]) {
			...
			umax(ret, seg.query(pos[chain[v]], pos[v]+1));
		}
		if (d[u]>d[v]) swap(u,v);
		umax(ret, seg.query(pos[u], pos[v]+1));
		return ret;
	}
