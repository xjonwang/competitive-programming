...
int main() {
	...
	EACH(ecow, east) {
		EACH(ncow, north) {
			if (ecow.x < ncow.x && ecow.y > ncow.y && ncow.up) {
				if (ecow.y-ncow.y < ncow.x-ecow.x) {
					adj[indices[ncow.x]].pb(indices[ecow.x]);
					break;
				} else if (ecow.y-ncow.y > ncow.x-ecow.x) {
					adj[indices[ecow.x]].pb(indices[ncow.x]);
					ncow.up = 0;
				}
			}
		}
